...
\begin{itemize}
    ...
    \item Download accessible referenced documents.
    \item Remove web noise such as navigation text, advertisements, and unrelated page elements.
    \item Extract the main textual content of the documents.
    \item Split each document into passages of approximately 150--300 words.
\end{itemize}

\textbf{Topic Inference}

The generation system does not receive the original Wikipedia article title as input. Instead, the topic of the article is inferred automatically from the cited documents. This is done by extracting candidate phrases from the sources and selecting the most representative one.

Candidate topic phrases are obtained from
\begin{itemize}
    \item titles of the cited documents,
    \item named entities extracted from the text,
    \item and frequently occurring noun phrases.
\end{itemize}

These candidates are ranked using frequency and semantic similarity between embeddings, and the most central candidate is selected as the inferred topic of the article. This inferred topic is then used as the query for retrieving relevant passages.

\subsection*{Approach 1: Baseline}

The baseline system reconstructs an article using a simple retrieval and summarization pipeline.

\begin{enumerate}
    \item \textbf{Passage Embedding}

    Each text passage extracted from the cited documents will be converted into a vector representation using the embedding model:

    \texttt{sentence-transformers/all-MiniLM-L6-v2}

    \item \textbf{Vector Index}

    All passage embeddings will be stored in a vector index (e.g.\ FAISS) to enable efficient similarity search.

    \item \textbf{Passage Retrieval}
    ...
    \item \textbf{Article Generation}

    The retrieved passages are concatenated and summarized using a pretrained summarization model such as:
    ...
    The resulting summary is treated as the reconstructed article.
\end{enumerate}

This approach serves as a baseline and does not involve training new models.

\subsection*{Approach 2: Retrieval-Augmented Generation}

The second approach implements a more structured retrieval-augmented generation (RAG) pipeline.

\begin{enumerate}
    \item \textbf{Embedding and Retrieval}

    The same embedding model (\texttt{all-MiniLM-L6-v2}) and vector index are used to retrieve relevant passages.

    \item \textbf{Topic Expansion}

    Additional queries are derived from the inferred topic by generating related subtopics (for example using noun phrase extraction or keyword expansion). These queries help retrieve passages covering different aspects of the topic.

    \item \textbf{Redundancy Reduction}

    Retrieved passages that are highly similar to each other may be filtered using cosine similarity or clustering in order to reduce repeated information.

    \item \textbf{Section Generation}

    Groups of retrieved passages are summarized independently using a stronger pretrained generation model such as:

    \begin{itemize}
        \item \texttt{flan-t5-base}
        \item \texttt{facebook/bart-large-cnn}
    \end{itemize}

    \item \textbf{Article Assembly}

    The generated summaries are combined into a final reconstructed article.
\end{enumerate}

This approach still relies on pretrained transformer models but improves the structure and grounding of the generated text through repeated retrieval steps.
