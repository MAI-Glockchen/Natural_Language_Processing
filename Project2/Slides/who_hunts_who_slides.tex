...
\begin{frame}{Method C --- OpenIE-style Relation Extraction}
  \begin{columns}[T,totalwidth=\textwidth]
    ...
    \small
      ...
      \good: variable-distance regex captures paraphrases missed by Method~B's window.

      \vspace{0.2em}
      \good: category expansion recovers edges when Wikipedia mentions a category (``insects'').

      \vspace{0.2em}
      \bad: generous distance windows allows subject- -relation--object triples that are not actually related.

      \vspace{0.2em}
      \bad: unconstrained category expansion can introduce false positives.
...
\begin{frame}{Summary}
  ...
  \begin{itemize}
    \item \textbf{Conclusion:} Combining explicit predation cues with constrained rule-based filtering achieved the best overall extraction performance.
  \end{itemize}

  \begin{center}
  \begin{tabular}{clll}
    \toprule
    & \textbf{Method} & \textbf{Core signal} & \textbf{Main characteristic} \\
    \midrule
  ...
  \centering
  \textbf{Method D achieved the highest Precision (0.889) and F1-score (0.762), while Method A achieved perfect Recall through exhaustive edge generation.}
