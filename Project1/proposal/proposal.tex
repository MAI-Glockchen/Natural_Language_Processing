...
The training set will be used to explore design choices and tune pipeline parameters such as passage length, the number of retrieved passages, topic inference strategies, redundancy filtering thresholds, and prompting strategies for the language model. The validation set will be used to compare candidate pipeline variants and select the final configuration. The test set will be kept untouched and used only for the final evaluation.

\subsection*{Shared Pipeline}

We propose two approaches -- the first simpler, the second more elaborate -- that will be compared to each other use the same initial processing steps:
...
\begin{enumerate}
    ...
    Web noise such as navigation text, advertisements, and unrelated page elements is removed. The main textual content is extracted and split into short, semantically coherent passages, for example individual sentences or small groups of neighboring sentences, so that the passages remain suitable for embedding-based retrieval.
    ...
    \href{https://huggingface.co/sentence-transformers/all-MiniLM-L6-v2}{\texttt{\seqsplit{sentence-transformers/all-MiniLM-L6-v2}}}\footnote{\label{ftnt:models}All suggested models used in this project were chosen because they are recent and can be locally run with modest hardware requirements.}. The resulting embeddings are stored in a vector index (e.g.\ FAISS) for efficient similarity search.
\end{enumerate}
...
\begin{itemize}
    \item \href{https://huggingface.co/google/gemma-2-2b}{\texttt{google/gemma-2-2b}}\footnote{\label{ftnt:models_gen}All suggested models used in this project were chosen because they are recent and can be locally run with modest hardware requirements.}
    \item \href{https://huggingface.co/Qwen/Qwen2.5-1.5B-Instruct}{\texttt{Qwen/Qwen2.5-1.5B-Instruct}}\footref{ftnt:models_gen}
    \item \href{https://huggingface.co/microsoft/Phi-3.5-mini-instruct}{\texttt{microsoft/microsoft/Phi-3.5-mini-instruct}}\footref{ftnt:models_gen}
\end{itemize}
